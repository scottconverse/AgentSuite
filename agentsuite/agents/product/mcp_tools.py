...
class ProductRunRequest(ProductAgentInput):
    """MCP-facing input for product_run. Adds optional run_id, supplies sane defaults."""

    run_id: str | None = None
    agent_name: str = "product"
    role_domain: str = "product"
    user_request: str = "run product agent"
    product_name: str = "My Product"
    target_users: str = "target users"
    core_problem: str = "problem to solve"
...
def _result_from_state(state: RunState, run_dir: Path) -> RunResult:
    primary = run_dir / "product-requirements-doc.md"
...
def register_tools(
    ...
    """Register the 5 default Product MCP tools (and 5 stage tools if expose_stages)."""

    def product_run(request: ProductRunRequest) -> RunResult:
        run_id = request.run_id or _now_id()
        agent = agent_class()
        product_input = ProductAgentInput(**request.model_dump(exclude={"run_id"}))
        state = agent.run(request=product_input, run_id=run_id)
        run_dir = output_root_fn() / "runs" / run_id
        return _result_from_state(state, run_dir)

    def product_resume(run_id: str, stage: Stage, edits: dict[str, Any] | None = None) -> RunResult:
        agent = agent_class()
        state = agent.resume(run_id=run_id, stage=stage, edits=edits or {})
        run_dir = require_run_dir(output_root_fn, run_id)
        return _result_from_state(state, run_dir)

    def product_approve(run_id: str, approver: str, project_slug: str) -> ApprovalResult | dict[str, Any]:
        agent = agent_class()
        try:
    ...
    def product_list_runs(project_slug: str | None = None) -> list[RunSummary]:
