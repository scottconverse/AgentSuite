...
def test_golden_design_primary_artifact_content_matches_snapshot(tmp_path: Path) -> None:
    """Design's visual-direction.md output must match the committed snapshot byte-for-byte.
    ...
    _, run_dir = _run_design(tmp_path)
    assert_artifact_exact(
        actual_path=run_dir / "visual-direction.md",
        fixture_path=SNAPSHOT_DIR / "visual-direction.md",
    )


def test_golden_design_qa_scores_within_tolerance(tmp_path: Path) -> None:
