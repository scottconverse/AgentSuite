...
def test_golden_marketing_json_artifact_structure(tmp_path: Path) -> None:
...
def test_golden_marketing_primary_artifact_content_matches_snapshot(tmp_path: Path) -> None:
    """Marketing's campaign-brief.md output must match the committed snapshot byte-for-byte.
    ...
    _, run_dir = _run_marketing(tmp_path)
    assert_artifact_exact(
        actual_path=run_dir / "campaign-brief.md",
        fixture_path=SNAPSHOT_DIR / "campaign-brief.md",
    )


def test_golden_marketing_qa_scores_within_tolerance(tmp_path: Path) -> None:
