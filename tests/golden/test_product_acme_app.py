...
def test_golden_product_primary_artifact_content_matches_snapshot(tmp_path: Path) -> None:
    """Product's product-requirements-doc.md output must match the committed snapshot byte-for-byte.
    ...
    _, run_dir = _run_product(tmp_path)
    assert_artifact_exact(
        actual_path=run_dir / "product-requirements-doc.md",
        fixture_path=SNAPSHOT_DIR / "product-requirements-doc.md",
    )


def test_golden_product_qa_scores_within_tolerance(tmp_path: Path) -> None:
